...
def gex_invalidated(df: pd.DataFrame, or_high: float, or_low: float, n: int) -> bool:
    """Failed-breakout exit: the last n 1-min closes are all back INSIDE the 15-min
    opening range — the breakout that triggered the entry has been reclaimed."""
    if n <= 0 or df.empty or len(df) < n:
        return False
    closes = df['close'].tail(n)
    return bool(((closes >= or_low) & (closes <= or_high)).all())
